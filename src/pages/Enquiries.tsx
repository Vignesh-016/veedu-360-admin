...
import { IconBuildingEstate, IconHomeHeart, IconRefresh, IconUsers } from '@tabler/icons-react';
...
export default function Enquiries() {
    ...
    return <div className="min-h-screen bg-[#fcfcfd] pb-12">
        ...
        <div className="border-b border-slate-200 bg-white"><div className="mx-auto flex max-w-[1600px] flex-col gap-5 px-4 py-8 sm:px-6 lg:px-8 md:flex-row md:items-center md:justify-between">
            ...
            <button onClick={load} className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50"><IconRefresh size={17} /> Refresh</button>
        </div></div>
        <main className="mx-auto max-w-[1600px] px-4 py-8 sm:px-6 lg:px-8">
            <div className="mb-6 flex flex-wrap gap-3 rounded-2xl border border-slate-100 bg-white p-4 shadow-sm"><select value={type} onChange={e => setType(e.target.value as CustomerEnquiryType | '')} className="rounded-lg border border-slate-200 px-3 py-2 text-sm"><option value="">All enquiry types</option><option value="TENANT">Tenant</option><option value="OWNER">House owner</option></select><select value={status} onChange={e => setStatus(e.target.value as CustomerEnquiryStatus | '')} className="rounded-lg border border-slate-200 px-3 py-2 text-sm"><option value="">All statuses</option><option value="NEW">New</option><option value="CONTACTED">Contacted</option><option value="CLOSED">Closed</option></select><span className="self-center text-sm text-slate-500">{enquiries.length} enquiry{enquiries.length === 1 ? '' : 'ies'}</span></div>
